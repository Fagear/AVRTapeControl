...
void mech_crp42602y_static_mode(uint8_t in_ttr_features, uint8_t in_srv_features, uint8_t in_sws, uint8_t *tacho, uint8_t *usr_mode, uint8_t *play_dir)
{
	if(u8_crp42602y_mode==TTR_42602_MODE_STOP)
	{
		...
		else 
		{
			...
			if((in_ttr_features&TTR_FEA_STOP_TACHO)!=0)
			{
		...
		if((*tacho)>TACHO_42602_PLAY_DLY_MAX)
		{
			...
			if((in_ttr_features&TTR_FEA_REV_ENABLE)!=0)
			{
				...
				if((in_srv_features&SRV_FEA_PB_AUTOREV)!=0)
				{
					...
					else if((u8_crp42602y_mode==TTR_42602_MODE_RC_REV)&&((in_srv_features&SRV_FEA_PB_LOOP)!=0)&&((in_sws&TTR_SW_NOREC_FWD)==0))
					{
						// Reverse ops: enabled, auto-reverse enabled.
...
void mech_crp42602y_state_machine(uint8_t in_ttr_features, uint8_t in_srv_features, uint8_t in_sws, uint8_t *tacho, uint8_t *usr_mode, uint8_t *play_dir)
{
	...
	if(u8_crp42602y_trans_timer==0)
	{
		...
		else
		{
			...
			mech_crp42602y_static_mode(in_ttr_features, in_srv_features, in_sws, tacho, usr_mode, play_dir);
		}
