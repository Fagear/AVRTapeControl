...
void mech_tanashin_state_machine(uint8_t in_features, uint8_t in_sws, uint8_t *taho, uint8_t *usr_mode, uint8_t *play_dir)
{
	...
	if(u8_tanashin_trans_timer==0)
	{
		...
		else if(u8_tanashin_target_mode!=u8_tanashin_mode)
		{
			...
			UART_add_string("TARGET!=CURRENT : "); mech_tanashin_UART_dump_mode(u8_tanashin_target_mode); UART_add_string(" != "); mech_tanashin_UART_dump_mode(u8_tanashin_mode); UART_add_flash_string((uint8_t *)cch_endl);
		...
		else if(mech_tanashin_user_to_transport((*usr_mode))!=u8_tanashin_target_mode)
		{
			...
			UART_add_string("USER!=TARGET : "); UART_dump_user_mode((*usr_mode)); UART_add_string(" != "); mech_tanashin_UART_dump_mode(u8_tanashin_target_mode); UART_add_flash_string((uint8_t *)cch_endl);
		...
		else
		{
			...
			else if(u8_tanashin_mode==TTR_TANA_MODE_PB_FWD)
			{
				...
				if((*taho)>TACHO_TANA_PLAY_DLY_MAX)
				{
			...
			else if((u8_tanashin_mode==TTR_TANA_MODE_FW_FWD)||(u8_tanashin_mode==TTR_TANA_MODE_FW_REV))
			{
				...
				// Check tachometer timer.
				if((*taho)>TACHO_TANA_FWIND_DLY_MAX)
				{
					// No signal from takeup tachometer for too long.
					// Perform auto-stop.
					u8_tanashin_target_mode = TTR_TANA_MODE_STOP;
	...
	else
	{
		...
		if(u8_tanashin_trans_timer==0)
		{
			...
			if(u8_tanashin_mode!=TTR_TANA_MODE_HALT)
			{
				//sprintf(u8a_buf, "TRFIN|%01u>%01u\n\r", u8_tanashin_mode, u8_tanashin_target_mode);
				UART_add_string("Mode transition done: "); mech_tanashin_UART_dump_mode(u8_tanashin_mode); UART_add_string(" > "); mech_tanashin_UART_dump_mode(u8_tanashin_target_mode); UART_add_flash_string((uint8_t *)cch_endl);
	...
	// Debug output.
	//OCR1AL = (u8_tanashin_mode+1)*(255/TTR_MODE_MAX);
